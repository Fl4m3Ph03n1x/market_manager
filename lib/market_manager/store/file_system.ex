defmodule MarketManager.Store.FileSystem do
  ...
  @impl Store
  def get_products_from_syndicate(_syndicate), do: {:ok, []}

  @impl Store
  def save_order(_order_id), do: {:ok, :order_saved}

  @impl Store
  def delete_order(_order_id), do: {:ok, :order_deleted}
end

defmodule MarketManager.Store.Old.Implementation do
  @moduledoc """
  """
  ...
  @spec activate(String.t()) ::
          {:error, atom | Jason.DecodeError.t()}
          | {:ok, :success}
          | {:partial_success, [String.t()], [any]}
          | {:error, :syndicate_not_found | :unable_to_place_requests, any}
  ...
  @spec deactivate(String.t()) ::
          {:error, atom | Jason.DecodeError.t()}
          | {:ok, :success}
          | {:partial_success, [String.t()], [any]}
          | {:error, :syndicate_not_found | :unable_to_delete_orders, any}
  ...
  defp make_requests(products),
    do:
      ...
      |> Enum.split_with(fn {status, _resp} -> status == :ok end)
